...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell: Cell, count: int):
        ...
        self.mark_safe(cell)
        neighbors = set([(cell[0] + i, cell[1] + j)
                         for i in [-1, 0, 1]
                         for j in [-1, 0, 1]
                         if (i, j) != (0, 0) and
                            (cell[0] + i >= 0) and (cell[0] + i < self.height) and
                            (cell[1] + j >= 0) and (cell[1] + j < self.width)
                         ])
        sentence = self.remove_known_cells(neighbors, count)
        self.incorporate_sentence(sentence)
        self.reduce_kb()
    ...
    def make_safe_move(self):
        ...
        """
        choices = [cell for cell in self.safes if cell not in self.moves_made]
